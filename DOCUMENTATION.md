...
|-----------|-----------------|
| **RotationalGridService / LimitOrderManager** | Механика: limit-ордера на добор и reduce-only на разгрузку |
| **LLM** | Только выбор символа для auto-open; добор/разгрузка по limit-ордерам, без LLM |

Сетка работает **независимо от LLM**: добор — limit на уровнях, разгрузка — reduce-only. При открытии новой позиции (auto-open) используем 1 слой (max_position / max_layers). `rotation_always_active=true` — сетка всегда выставляет ордера, без привязки к режиму рынка.
...
| `rotation_always_active` | Сетка всегда активна (независимо от режима рынка) |
